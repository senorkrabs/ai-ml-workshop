...
let aid = $('#access_id').val();
let skey =$('#secret_key').val();

AWS.config = { "accessKeyId": aid, "secretAccessKey": skey, "region": "us-east-1" };
    ...
    comprehend.detectSentiment(params, function(err, data) {
        if (err) console.log(err, err.stack);
            ...
            if (data.Sentiment == 'NEGATIVE') {
                $('#sent_face').html(face_s);
            } else if ( data.Sentiment == 'POSITIVE') {
                $('#sent_face').html(face_h);
            } else {
                $('#sent_face').html(face_n);
